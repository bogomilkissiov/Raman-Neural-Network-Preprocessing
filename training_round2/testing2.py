...
# -----------------------------------------------------------------------------
# Look for test_spectra2 within the local training_round2 folder or relative to project root
test_spectra_dir = os.path.join(SCRIPT_DIR, "test_spectra2")
if not os.path.exists(test_spectra_dir):
    test_spectra_dir = os.path.join(PROJECT_ROOT, "training_round2", "test_spectra2")
if not os.path.exists(test_spectra_dir):
...
print(f"Loading {len(test_files)} test dataset files:\n")
for f in test_files:
    print(f" - {f}")

if not test_files:
    raise FileNotFoundError(f"No test dataset files found in: {test_spectra_dir}")
...
# Stack all parts vertically into continuous 2D matrices
...
# -----------------------------------------------------------------------------
# Generate wavenumber grid matching the spectral length (e.g. 0 to 1015 cm^-1, step size 1)
...
print(f"Spectra objects initialized ({pure_intensities.shape[0]:,} spectra, {num_wavenumbers} points).\n")
...
print(f"Loading neural network on device: {device}...\n")

# 1. Instantiate the architecture matching spectral dimension
model = DualSupervisedNet(input_length=num_wavenumbers).to(device)

# 2. Load the trained model weights for round 2
model_weights_path = os.path.join(SCRIPT_DIR, "dual_supervised_resnet2.pth")
if not os.path.exists(model_weights_path):
    model_weights_path = os.path.join(PROJECT_ROOT, "dual_supervised_resnet2.pth")
if not os.path.exists(model_weights_path):
    model_weights_path = "dual_supervised_resnet2.pth"

model.load_state_dict(torch.load(model_weights_path, map_location=device, weights_only=True))
print(f"Neural network loaded successfully from {model_weights_path}.\n")
...
class SpectrumSlideshow:
    ...
    """
    def __init__(self, wavenumbers, pure_mat, raw_mat, conv_mat, nn_mat, num_samples=20):
        self.wavenumbers = wavenumbers
        
        # Pick sample subset from the test set
    ...
    def update_plot(self):
        ...
        ax_nn.set_title("Neural Network Preprocessed (Dual-Supervised ResNet Round 2)", fontsize=11, fontweight="bold")
...
# Launch the interactive viewer for 20 synthetic spectra
SpectrumSlideshow(
...
# -----------------------------------------------------------------------------
# 9. RUN ON REAL EXPERIMENTAL DATA
# -----------------------------------------------------------------------------
wdf_filename = "2s_100lp_map-2 (1).wdf"
wdf_path = os.path.join(PROJECT_ROOT, wdf_filename)
if not os.path.exists(wdf_path):
    wdf_path = wdf_filename

real_raw_data_full = fl.wdf_to_spectra(wdf_path)

# Slice out only the first 20 spectra to save memory and speed up computation
if isinstance(real_raw_data_full, list):
    real_raw_data = spectra(real_raw_data_full[:20])
else:
...
# Resample intensity matrix to match the exact input length expected by the neural network
...
class RealSpectrumSlideshow:
    ...
    Shows 4 stitched subplots in Gruvbox style:
      ...
      - Neural Network Fully Preprocessed (Dual-Supervised ResNet Round 2)
    """
    ...
    def update_plot(self):
        ...
        # 1. Exact Input to Neural Network (Normalized, Shifted & Resampled)
        ...
        ax_nn.set_title("Neural Network: Fully Preprocessed (Round 2)", fontsize=11, fontweight="bold")
        ax_nn.set_xlabel("Wavenumber (cm⁻¹)")
        ax_nn.legend(loc="upper right")
