...
@process_spectra_data
def remove_baseline(wavenumbers, intensities, order=4, method='combo', l_=100):
    """
...
def preprocess_pipeline(
    ...
    Runs them through the selected preprocessing steps, modifying the objects in place.
    
    Parameters:
        data: spectrum, list[spectrum], or spectra object.
        despike (bool): If True, run despike_spectra. Default is True.
        denoise (bool): If True, run denoise_spectra. Default is True.
        baseline (bool): If True, run remove_baseline. Default is True.
        normalize (bool): If True, run normalize_spectra. Default is True.
        shift (bool): If True, run shift_to_zero. Default is True.
    """
    if despike:
